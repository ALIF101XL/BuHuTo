...
if 'buhuto' in sys.modules:
sys.modules['buhuto'].stop_event = stop_event
...
if 'buhuto' in sys.modules:
sys.modules['buhuto'].log = gui_log
...
gui_log("error", f"Process interrupted or failed: {
  str(e)}")
finally:
event.clear()
...
mode = f"Specific Modules: {
  ','.join(final_modules_list)}"
...
count = 50
gui_log("info", f"Starting proxy downloader with count: {
  count
}")
threading.Thread(
...
print(f"An unexpected environment error occurred: {
  e
}")
sys.exit(1)
except Exception as e:
