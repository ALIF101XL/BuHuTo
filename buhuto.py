...
def loading_animation():
    log("run", "Initializing BuHuTo V1.0...")
    time.sleep(1)
...
def scan_single_url(full_scan=True, url=None, module=None, include_ssrf=True):
    ...
    scan_output_dir = os.path.join(PROJECT_ROOT, "scan_results", f"scan_{_get_timestamp()}")
    os.makedirs(scan_output_dir, exist_ok=True)

    command = f"python3 {os.path.join(SCRIPT_DIR, 'misc', 'tools.py')} {shlex.quote(url)} --output-dir {shlex.quote(scan_output_dir)} --yes"
    if full_scan:
        command += " --deep-scan"
        if is_cli_call:
            confirm = input("Do you want to include internal SSRF scanning? (y/n, default: n): ").lower()
            if confirm == 'n':
                include_ssrf = False
        if not include_ssrf:
        ...
        if module is None:
            ...
            additional_module = input(f"{Fore.YELLOW}Enter additional module (e.g., xss, sql, or press Enter for none): {Style.RESET_ALL}").strip()
